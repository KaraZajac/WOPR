...
def engine_prior(criteria: dict) -> dict | None:
    ...
    if criteria["measure"] in ("terminates", "coup"):
        grain = "dyad" if criteria["measure"] == "terminates" else "country"
        if scope["kind"] != grain:
            return None
        sub = baserate.load_substrate()
        if scope["id"] not in sub[grain]:
            return None
...
def synthesize_question(criteria: dict) -> str:
    ...
    if c["measure"] == "coup":
        year = str(c["window"]["start"])[:4]
        return (
            f"Will Powell–Thyne record a coup attempt (successful or failed) "
            f"in {c['scope']['name']} during {year}?"
        )
...
def cmd_ask(args) -> None:
    scope, default_types = scope_from_args(args)
    types = args.types.split(",") if args.types else default_types
    if args.measure in ("terminates", "coup") and not args.year:
        raise SystemExit(f"{args.measure} questions are calendar-year questions: use --year")
...
def main(argv: list[str] | None = None) -> None:
    ...
    scope_flags(p)
    p.add_argument("--measure", choices=("deaths", "acd-active", "terminates", "coup"), default="deaths")
    ...
    scope_flags(p)
    p.add_argument("--measure", choices=("deaths", "events", "terminates", "coup"), default="deaths")
